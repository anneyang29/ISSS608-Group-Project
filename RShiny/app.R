...
read_prepared_data_bundle <- function(rds_path) {
  ...
  prepared <- readRDS(rds_path)

  if (
    is.list(prepared) &&
    all(c("base_data", "time_data") %in% names(prepared)) &&
    !is.null(prepared$base_data) &&
    !is.null(prepared$time_data)
  ) {
    return(prepared)
  }
...
data_bundle <- read_prepared_data_bundle(base_data_path)
...
if (is.null(data_bundle)) {
  ...
  if (is.na(customer_csv_path) || is.na(tx_csv_path)) {
    stop(
      paste0(
        "Unable to find a valid shiny_base_data.rds bundle and required CSV files. ",
  ...
  if (!is.na(base_data_path)) {
    message("Legacy or incomplete shiny_base_data.rds detected. Rebuilding prepared data from CSV files.")
  }
...
data_note_ui <- tags$div(
  ...
  tags$details(
    ...
    tags$div(
      ...
      HTML(paste0(
        "This dashboard uses the COFINFAD customer and transaction data prepared for the project. ",
        "Several behavioural metrics such as transaction frequency and volume are engineered from the raw transactions and cached in <i>shiny_base_data.rds</i> for faster loading. ",
        "If the cache is missing or outdated, the app rebuilds it from the CSV files automatically."
      ))
    )
  )
)
